...
#### 1.2.1 `new`和`delete`的底层原理

+ **`new`的底层原理**
当我们使用`Complex* pc = new Complex(1, 2)`语句时，编译器会翻译为：
...
这里的`operator new`如果没有被重载，那么它使用的是全局的函数，它的定义为：
...
_THROW0()
{
    ...
    {
        // buy more memory or returrn null pointer
        _TRY_BEGIN
...
+ `delete`的底层原理
+ 当我们使用`Complex* pc = new Complex(1, 2); delete pc`语句时，编译器会翻译为：

```cpp
pc->~Complex(); // 调用 Complex 的析构函数
::operator delete(p); // 释放内存
```

而`operator delete`如果没有被重载，那么它使用的是全局的函数，它的定义为：
...
![](./image/resultMemoryPrimitives_g++_2.png)
